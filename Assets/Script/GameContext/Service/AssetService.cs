...
{
    void LoadPrefab(string address,Action<GameObject> onLoaded, Action onFailed = null);
    void LoadMaterial(string address, Action<Material> onLoaded, Action onFailed = null);
    void LoadData(string address, Action<ScriptableObject> onLoaded, Action onFailed = null);
}
...
public class AssetLoadingTask
{
    ...
    public AssetLoadingTask(string inAddress, AssetService inService)
    {
        _address = inAddress;
        _isValid = true;
        _service = inService;
...
public class AssetService : IAssetLoader, IDisposable
{
    ...
    public void LoadPrefab(string address, Action<GameObject> onLoaded, Action onFailed = null)
    {
        LoadAsset<GameObject>(
            ...
            task =>
            {
                onLoaded?.Invoke(task.GetAsset<GameObject>());
            },
            onFailed);
    }

    public void LoadMaterial(string address, Action<Material> onLoaded, Action onFailed = null)
    {
        LoadAsset<Material>(
            address,
            task =>
            {
                onLoaded?.Invoke(task.GetAsset<Material>());
            },
            onFailed);
    }

    public void LoadData(string address, Action<ScriptableObject> onLoaded, Action onFailed = null)
    {
        LoadAsset<ScriptableObject>(
            address,
            task =>
            {
                onLoaded?.Invoke(task.GetAsset<ScriptableObject>());
            },
            onFailed);
    }
